...
class ElastisearchPost extends BasicBolt {
    ...
    async receive(message, stream_id, callback) {
        let self = this;
        const { oer_materials: { material_id, title, description, language, creation_date, retrieved_date, type, mimetype, license }, material_contents, features_public, urls: { material_url, provider_uri: website_url }, provider_token } = message;
        let contents = [];
        ...
        self._pg.select({ token: provider_token }, "providers", (xe, results) => {
            const { id: provider_id, name: provider_name, domain: provider_url } = results[0];
            const record = {
                material_id,
                title,
                description,
