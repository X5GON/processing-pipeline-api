...
class StorePGMaterialComplete {
    ...
    receive(message, stream_id, callback) {
        let self = this;
        const { oer_materials, material_contents, features_public, urls, provider_token } = message;
        ...
        self._pg.insert(oer_materials, "oer_materials", (error, result) => {
            ...
            tasks.push((xcallback) => {
                ...
                self._pg.select({ token: provider_token }, "providers", (xe, results) => {
                    ...
                    let material_url = Object.assign({ url: urls.material_url, material_id }, provider_id && { provider_id });
                    let provider_uri = Object.assign({ url: urls.provider_uri }, provider_id && { provider_id });
                    // set url list
